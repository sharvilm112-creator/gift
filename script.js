...
const obs=new IntersectionObserver(entries=>{entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('visible');if(e.target.closest('#story')){let tl=document.querySelector('.timeline-line');if(tl)tl.classList.add('grown')}if(e.target.classList.contains('letter-line')){let d=e.target.dataset.delay||0;e.target.style.transitionDelay=d*.3+'s'}}})},{threshold:.15,rootMargin:'0px 0px -50px 0px'});
document.querySelectorAll('.fade-reveal,.timeline-event,.letter-line,.gallery-item').forEach(el=>obs.observe(el));

// Unified 3D Tilt for Desktop & Mobile
document.querySelectorAll('.tilt-card').forEach(card=>{
const handleMove=(cx,cy)=>{let r=card.getBoundingClientRect(),x=cx-r.left,y=cy-r.top,rx=(y-r.height/2)/(r.height/2)*-8,ry=(x-r.width/2)/(r.width/2)*8;card.style.transform='perspective(600px) rotateX('+rx+'deg) rotateY('+ry+'deg) scale(1.03)'};
const handleEnd=()=>{card.style.transform='perspective(600px) rotateX(0) rotateY(0) scale(1)'};
card.addEventListener('mousemove',e=>handleMove(e.clientX,e.clientY));
card.addEventListener('touchmove',e=>{if(e.touches[0])handleMove(e.touches[0].clientX,e.touches[0].clientY)},{passive:true});
card.addEventListener('mouseleave',handleEnd);
card.addEventListener('touchend',handleEnd);
});

// Unified Magnetic Buttons for Desktop & Mobile
document.querySelectorAll('.magnetic-btn').forEach(btn=>{
const handleMove=(cx,cy)=>{let r=btn.getBoundingClientRect(),x=cx-r.left-r.width/2,y=cy-r.top-r.height/2;btn.style.transform='translate('+x*.2+'px,'+y*.2+'px) scale(1.05)'};
const handleEnd=()=>{btn.style.transform='translate(0,0) scale(1)'};
btn.addEventListener('mousemove',e=>handleMove(e.clientX,e.clientY));
btn.addEventListener('touchmove',e=>{if(e.touches[0])handleMove(e.touches[0].clientX,e.touches[0].clientY)},{passive:true});
